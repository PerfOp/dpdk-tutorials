...
bool prepare_memory_pool(rte_mempool *mempool, const BenchParam& benchparam) {
    ...
    if (!mempool) {
        std::cerr << "Unable to lookup mempool: " << KNicPoolName
                  << std::endl;
        return false;
    }
    ...
    if (i != KNicBufCount) {
        std::cerr << "Not all the memory buffers are available in mempool: "
                  << KNicPoolName << std::endl;
        return false;
    }
    ...
    for (uint32_t j = 0; j < memory_buffers.size(); ++j) {
        ...
        memcpy(&ipv4_hdr->src_addr, benchparam.src_ip,
        ...
        memcpy(&ipv4_hdr->dst_addr, benchparam.dst_ip,
        ...
        std::string sample_data=
            "This is a sample data generated by a DPDK application ..."+std::to_string(j);
