...
bool IOProcess::InitPrimaryResource() {
    ...
    //
    // DataPool: 1024*1024 buffers, 1024 bytes per buffer
    log_error("bypass:","-------------------------------->");
    log_error("bypass:","Init the pools for data and cmds.");
    if (!m_dataPool.create_pool(KDataPoolName, 1024*1024, 1024, rte_socket_id())) {
        printf_error("Cannot create data pool:%s!\n", KDataPoolName.c_str());
        exit(1);
    } else {
        bypass_log_error("Pool created: %s", KDataPoolName.c_str());
    }

    if (!m_dataRing.create_ring(KDataRingName, 512*1024, rte_socket_id())) {
        printf_error("Cannot create data ring:%s \n", KDataRingName.c_str());
        exit(1);
    } else {
    ...
    if (!m_cmdPool.create_pool(KCmdPoolName, 1024, 512, rte_socket_id())) {
        ...
        bypass_log_error("Pool created: %s", KCmdPoolName.c_str());
    }

    if (!m_cmdRing.create_ring(KCmdRingName, 512*1024, rte_socket_id())) {
        printf_error("Cannot create cmd ring:%s \n", KCmdRingName.c_str());
        exit(1);
    } else {
        bypass_log_error("Ring created: %s", KCmdRingName.c_str());
    }
    bypass_log_error("");
    return true;
}
