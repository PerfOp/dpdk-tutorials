...
bool IOProcess::InitPrimaryResource(const std::string poolname,
        const std::string ringname) {
    // Register a timestamp dynamic field.
    m_dataQueue = new ExchangeQueue();
    ...
    if (!m_dataQueue->create_pool(poolname, rte_socket_id())) {
        printf_error("Cannot create pool!\n");
        exit(1);
    }

    if (!m_dataQueue->create_ring(ringname, 512, rte_socket_id())) {
...
int IOProcess::IO_loop() {
    ...
    while (!exit_indicator) {
        ...
        rte_mbuf *const packet = m_dataQueue->allocate_mbuf();
        if (!packet) {
            continue;
        }
        ...
        if (m_dataQueue->produce_packets(packet)) {
            total_tx_packets++;
...
bool NicProcess::InitNicResource(const std::string ringname) {
    // Register a timestamp dynamic field.
    m_attachDataQueue = new LiteQueue();
    // Primary process with look up for the ring buffer and receive the
    // packets generated by primary DPDK application. Lookup for the ring
    // buffer created by a primary application.
...
int NicProcess::CMD_loop() {
    ...
    while (!exit_indicator) {
        ...
        rx_count = m_attachDataQueue->consume_packets(rx_packets, 1);
