...
bool IOProcess::InitPrimaryResource(const std::string poolname,
    ...
    if (!m_dataPool.create_pool(poolname, rte_socket_id())) {
        printf_error("Cannot create pool!\n");
        exit(1);
    }

    if (!m_dataRing.create_ring(ringname, 512, rte_socket_id())) {
...
bool NicProcess::InitNicResource(const std::string ringname) {
    ...
    if (!m_dataRing.attach_ring(ringname)) {
